...
2. ✍️ Uses an instruction-tuned language model served through OpenRouter to generate the SQL query.
...
This project uses OpenRouter instead of directly integrating with a single LLM provider.

Benefits include:

- Access to multiple models through a unified API.
- Ability to experiment with free and paid models.
...
|---|---|
| 🌊 **Context window overflow** | A real warehouse can have 50–200 tables. The full schema — table names, column names, types, foreign keys — can easily exceed the LLM's context limit. |
...
1. **At startup:** Embed every table description into ChromaDB using a configurable embedding model so only the most relevant schema context is retrieved during query generation. Each table becomes a point in high-dimensional space. 📌
...
│  │     ▼                                                │   │
│  │  [4] Gemma / OpenRouter-supported LLM                |   |
|  |   (temperature=0) ◄── OpenRouter API                 │   │
│  │     │   (generate SQL)                               │   │
│  │     ▼                                                │   │
│  │  [5] hitl_guard.py                                   │   │
...
The user's question is transformed into vector representations using the configured embedding model so semantically similar schema descriptions can be retrieved efficiently..
...
**Step 4 → Ask Gemma-4 🤖**
The prompt is sent to Gemma-4 with `temperature=0` (fully deterministic — no creative variation in SQL). The model returns a raw SQL string.
...
```bash
cp .env.example .env
# ✏️ Edit .env and set your OPENROUTER_API_KEY
```
